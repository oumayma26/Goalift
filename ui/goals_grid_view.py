...
Grille responsive de cards Goal.
"""
...
class GoalsGridView(ctk.CTkFrame):
    """Vue grille de goals — 3 colonnes égales, responsive."""
    ...
    def _build_header(self) -> None:
        ...
        self.header.grid(row=0, column=0, sticky="ew", padx=20, pady=(20, 15))
    ...
    def refresh(self, goals: List[Goal]) -> None:
        ...
        for idx, goal in enumerate(goals):
            row = idx // 3
            col = idx % 3
            self._create_goal_card(goal, row, col)
    ...
    def _create_goal_card(self, goal: Goal, row: int, col: int) -> None:
        """Crée une card Goal — grid() dans grid_container, pack() à l'intérieur."""
        progress = self.service.get_goal_progress(goal.id)
        percentage = progress["percentage"]
        goal_color = goal.color or "#3B82F6"

        # Card : grid() dans grid_container, pack() interne
        card = ctk.CTkFrame(
            self.grid_container,
            fg_color="#FFFFFF",
            corner_radius=16,
        ...
        if goal.image_path and os.path.exists(goal.image_path):
            try:
                img_with_shadow = self._create_image_with_shadow(
                    goal.image_path, width=296, height=130, radius=12
                )
                ctk_img = ctk.CTkImage(light_image=img_with_shadow, dark_image=img_with_shadow, size=(296, 130))
        ...
        fill_width_pct = max(0.0, min(1.0, percentage / 100.0))
        bar_fill = ctk.CTkFrame(bar_bg, height=6, corner_radius=3, fg_color=percent_color)
        ...
        ctk.CTkLabel(status_frame, text="●", font=ctk.CTkFont(size=10), text_color=dot_color).pack(side="left", padx=(0, 5))
        ctk.CTkLabel(status_frame, text=goal.status, font=ctk.CTkFont(size=11), text_color="#64748B").pack(side="left")
    ...
    def _build_placeholder(self, parent, goal: Goal, color: str) -> None:
        placeholder = ctk.CTkFrame(parent, fg_color=color, corner_radius=12)
